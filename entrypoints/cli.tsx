...
if (typeof MACRO === 'undefined') {
  ;(globalThis as any).MACRO = {
    VERSION: '@konstantindevv edition',
...
// Codeus dev: Shift+Tab debug permission mode is gated by compile-time DEBUG_MODE
// or CLAUDE_CODE_DEBUG_PERMISSION_MODE. Bundled dev runs usually lack the former;
// default the latter so the carousel includes Debug (set to 0 to disable).
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (process.env.CLAUDE_CODE_DEBUG_PERMISSION_MODE === undefined) {
  // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
  process.env.CLAUDE_CODE_DEBUG_PERMISSION_MODE = '1'
}
...
async function main(): Promise<void> {
  ...
  // Capture prompt typing after main loads — setting raw mode before this import
  // leaves Windows terminals blank / "stuck" for the entire multi-second compile,
  // and disables line discipline (Ctrl+C) until Ink mounts.
  const {
...
void main().catch((err: unknown) => {
  ...
  // biome-ignore lint/suspicious/noConsole: intentional fatal CLI error
  console.error(`Codeus: ${msg}`);
